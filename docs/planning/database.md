...
**`unaccent` cho `products.name/description` và `categories.name` (PR09):** `ILIKE` không phân biệt hoa/thường nhưng vẫn phân biệt dấu tiếng Việt — tìm "chuot khong day" không khớp "Chuột không dây". Migration `AddProductsUnaccentSearch` chỉ bật extension `CREATE EXTENSION unaccent`; câu query bọc `unaccent(...)` cả 2 vế (cột lẫn tham số) ngay trong `ProductsService`/`CategoriesService`, không thêm index mới — giữ đúng tinh thần MVP ở trên (chấp nhận `ILIKE` chưa tăng tốc bởi index). Không tái dùng luôn cách làm sớm như `pg_trgm` ở trên vì `unaccent()` mặc định là STABLE (không phải IMMUTABLE), muốn đưa vào index phải viết thêm 1 wrapper function IMMUTABLE riêng — để lại cho lúc thực sự đo thấy cần. `GET /products?q=` xếp kết quả khớp `name` trước khớp chỉ ở `description` bằng 1 cột tính toán (`CASE WHEN ... THEN 0 ELSE 1 END`) `addSelect` kèm alias riêng rồi `orderBy` theo alias đó — không thể `orderBy` thẳng bằng raw SQL chứa `alias.column`, vì `getManyAndCount()` (join + `take`/`skip`) dùng lại chuỗi `orderBy` để dựng subquery phân trang theo alias, và TypeORM sẽ tưởng nhầm chuỗi raw đó là 1 tham chiếu `alias.column` cần lookup.
