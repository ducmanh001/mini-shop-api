...
| `C014` (PR06, áp dụng chung mọi service có sẵn)   | _"Direct instantiation of 'Logger'"_ ở `auth.service.ts` (và đã có sẵn trước đó ở `mail.processor.ts`, `notification-dispatcher.service.ts`, `all-exceptions.filter.ts`; `users.service.ts` thêm ở PR07; `categories.service.ts`/`products.service.ts` thêm ở PR09)                                                                                                                              | `new Logger(ClassName.name)` là pattern chính thức NestJS khuyến nghị cho field logger riêng của từng class — không phải service dependency cần qua DI như `Repository`/`ConfigService`. Bắt buộc DI ở đây sẽ phá vỡ context name tự động (`ClassName.name`) mà `Logger` dùng để gắn nhãn log.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
